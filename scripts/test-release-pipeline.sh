...
grep -q 'git commit -m "I release HyprMac v\$NEW_VERSION"' "$SCRIPT"
grep -q 'git -C "$TEMP_TAP" commit -m "I update HyprMac to v\$NEW_VERSION"' "$SCRIPT"
...
commit_line=$(grep -n 'git commit -m "I release HyprMac v\$NEW_VERSION"' "$SCRIPT" | cut -d: -f1)
