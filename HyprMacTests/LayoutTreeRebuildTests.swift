...
final class LayoutTreeRebuildTests: XCTestCase {
    ...
    func testSortPriorityWinsOverSavedLeafOrder() {
        // window 1 is pinned to the bottom-right end by its rule
        engine.sortPriority = { $0.windowID == 1 ? -1 : 0 }
        let saved = LayoutNode.split(override: nil, ratio: 0.6, userSet: true,
                                     left: leaf(1),
                                     right: .split(override: nil, ratio: 0.5, userSet: false, left: leaf(2), right: leaf(3)))
        ...
        XCTAssertEqual(outcome, .rebuilt(inserted: 0, refusedNewcomers: []))
        XCTAssertEqual(live()!.allWindows.map(\.windowID), [2, 3, 1])
        XCTAssertEqual(live()!.root.splitRatio, 0.6, "the saved shape stays, only tiles move")
    }

    func testRebuiltTreeAnswersFullHeightRule() {
        engine.fullHeight = { $0.windowID == 1 }
        let saved = LayoutNode.split(override: nil, ratio: 0.5, userSet: false, left: leaf(1), right: leaf(2))

        _ = engine.rebuildTree(forWorkspace: 1, screen: screen, from: saved,
                               windows: ids([1, 2]), applyFrames: true, resolve: byID)

        XCTAssertTrue(live()!.isFullHeight(windows[1]!), "the rebuilt tree must carry the engine's predicate")
    }

    func testAccordionScreenPublishesWithoutVerifiedSizing() {
        // every verified write fails, so only the accordion path can publish
        let refusing = makeEngine(refusingWritesFrameSizingIOFactory())
        refusing.accordionActive = { _ in true }
        let saved = LayoutNode.split(override: nil, ratio: 0.5, userSet: false, left: leaf(1), right: leaf(2))
...
private final class RebuildTestScreen: NSScreen {
    ...
    override var localizedName: String { "rebuild-test" }
    override var deviceDescription: [NSDeviceDescriptionKey: Any] {
        [NSDeviceDescriptionKey("NSScreenNumber"): NSNumber(value: 921000)]
    }
}
...
private final class RebuildOtherScreen: NSScreen {
    ...
    override var visibleFrame: NSRect { frame }
    // detached test screen: AppKit traps on both of these on macOS 26
    override var localizedName: String { "rebuild-other" }
    override var deviceDescription: [NSDeviceDescriptionKey: Any] {
        [NSDeviceDescriptionKey("NSScreenNumber"): NSNumber(value: 921001)]
    }
}
