...
private final class RestorerRig {
    ...
    var restorer: LayoutRestorer {
        LayoutRestorer(engine: engine, orchestrator: orchestrator, workspaceManager: workspaceManager,
                       ...
                       isSticky: { [stickyIDs] in stickyIDs.contains($0) },
                       linkedMonitors: workspaceManager.linkedMonitors,
...
final class LayoutRestorerTests: XCTestCase {
    ...
    // MARK: - fork: sticky windows and linked monitors

    func testStickyWindowIsNeverMovedBack() {
        let rig = RestorerRig()
        let visible = rig.visible(on: 0)
        let hidden = rig.hidden(on: 0)
        rig.fill(visible, [1, 2])
        ...
        let outcome = rig.restore([hidden: split(leaf(1), leaf(2))])

        XCTAssertEqual(outcome.moved, [1: hidden])
        XCTAssertEqual(rig.workspace(of: 2), visible, "the sticky reconcile owns where a sticky window sits")
        rig.assertEachWindowInOneTree()
    }

    func testLinkedCaptureSavesMembershipWithoutShape() {
        let rig = RestorerRig(screenCount: 2)
        rig.workspaceManager.linkedMonitors = true
        rig.workspaceManager.initializeMonitors()
        let ws = rig.visible(on: 0)
        rig.fill(ws, [1, 2], screen: rig.screens[0])
        rig.fill(ws, [3], tile: false)
        _ = rig.engine.tileWindows([rig.windows[3]!], onWorkspace: ws, screen: rig.screens[1])

        let layout = rig.restorer.capture().first { $0.workspace == ws }

        XCTAssertNil(layout?.root, "a per-screen tree is not the linked workspace's shape")
        XCTAssertEqual(layout?.unplaced, [1, 2, 3].map { RestorerRig.ref($0) })
    }

    func testLinkedRestoreMovesWithoutRebuildingShapes() {
        let rig = RestorerRig(screenCount: 2)
        rig.workspaceManager.linkedMonitors = true
        rig.workspaceManager.initializeMonitors()
        let ws = rig.visible(on: 0)
        let other = Constants.workspaceRange.first { $0 != ws }!
        rig.fill(ws, [1, 2], screen: rig.screens[0])
        rig.fill(other, [3], tile: false)

        let outcome = rig.restore([ws: split(leaf(1), leaf(2))],
                                  unplaced: [ws: [RestorerRig.ref(3)]])

        XCTAssertEqual(outcome.moved, [3: ws])
        XCTAssertTrue(outcome.rebuilt.isEmpty)
        XCTAssertEqual(rig.workspace(of: 3), ws)
    }
}
