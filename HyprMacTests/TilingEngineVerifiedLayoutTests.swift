    ...
    func testOrdinaryLayoutEntryPointsRestoreFramesAfterUnknownRead() throws {
        ...
        for entryPoint in OrdinaryEntryPoint.allCases {
            ...
            case .addWindowRetile:
                ...
                // inside the screen's usable frame whatever the display
                // arrangement — an original parked off-screen is not a
                // restoration target and would skip the rollback under test
                let usable = DisplayManager().cgRect(for: screen)
                trace.frames[third.windowID] = CGRect(x: usable.minX + 100, y: usable.minY + 100,
    ...
    func testReentrantStateChangesSupersedeActiveLayoutWithoutOldRollback() throws {
        ...
        for change in ReentrantStateChange.allCases {
            ...
            trace.onFirstRead = {
                ...
                case .padding: engine.outerPadding = engine.outerPadding + 1
                case .removeWindow: engine.removeWindowID(second.windowID)
                case .display: engine.handleDisplayChange(currentScreens: [], homeScreensForWorkspace: { _ in [] })
                case .prepareToggle:
                    _ = engine.prepareToggleSplitLayout(first, onWorkspace: 1, screen: screen)
...
private func timeoutRecoveryFixture(
    ...
    let targets = Dictionary(uniqueKeysWithValues: tree.layout(
        in: usable, gap: TilingConfig.defaultGap, padding: OuterPadding(uniform: TilingConfig.defaultOuterPadding)
    ).map { ($0.0.windowID, $0.1) })
